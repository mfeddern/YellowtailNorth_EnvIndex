...
# load data files ####
temp_daily = data.table(read.csv(paste0(data_dir, "glorys-daily-temperature.csv")))
dt = temp_daily
# fix date
# get year
...
cst_monthly = data.table(read.csv(paste0(data_dir, "glorys-monthly-means-cst.csv")))
lst_monthly = data.table(read.csv(paste0(data_dir, "glorys-monthly-means-lst.csv")))
mld_monthly = data.table(read.csv(paste0(data_dir, "glorys-monthly-means-mld.csv")))
...
hci1 = data.table(read.csv( paste0(data_dir, "ei_hci_rgn1_M.csv" )))
...
hci2 = data.table(read.csv( paste0(data_dir, "ei_hci_rgn2_M.csv" )))
...
# LUSI (annual)
lusi = data.table(read.csv( paste0(data_dir, "cciea_OC_LUSI.csv" )))
lusi = lusi[-1,]
lusi$year = lubridate::year(lusi$time)
lusi$lusi <- as.numeric(as.character(lusi$lusi))
lusi_annual = lusi[ year %in% c(1993:2018), list(lusi_annual = mean(lusi, na.rm=TRUE)), by='year']
